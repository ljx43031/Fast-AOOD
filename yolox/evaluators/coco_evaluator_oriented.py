...
def per_class_AP_table(coco_eval, class_names=COCO_CLASSES, headers=["class", "AP"], colums=6):
    ...
    for idx, name in enumerate(class_names):
        ...
        precision = precisions[0, :, idx, 0, -1]  # 05
        # precision = precisions[:, :, idx, 0, -1] # 05:95
...
class COCOEvaluator_Oriented:
        ...
        for (output, img_h, img_w, img_id) in zip(
            ...
            image_wise_data.update({
                int(img_id): {
                    ...
                    "rotates": [angle.numpy().item() for angle in angles],
                }
            })
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        if len(data_dict) > 0:
            ...
            cocoEval = COCOeval_Oriented(cocoGt, cocoDt, annType[1])
            cocoEval.evaluate()
            cocoEval.accumulate()
